...
import { type DistatchTasks, TYPE_DISPATCH_TASKS, taskReducer } from './reducers/task.reducer'
import Form from './components/Form'
import TaskForm from './components/TaskForm'
import Sort from './components/Sort'
import { useModal } from './components/Modal'
import { type ITask, type KeysRequiredType } from './types/task'
import { type SortOptiosType } from './types/sort'
...
function App () {
  ...
  const handlerSubmitEdit = (formData: ITask) => {
    for (const key of keysRequired) {
      if (!formData[key]) {
        alert(`Selecciona o completa el campo: ${key}`); return
      }
    }
    if (taskToEdit != null) {
      dispatchTasks({
